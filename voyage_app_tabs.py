...
class VoyageAppTabs:  # (0)
    ...
    def initial_load_df(self) -> pd.DataFrame:  # (4)
        ...
        try:  # (8)
            ...
            rename_map = {info["full"]: key for key, info in FIELDS_CONFIG.items()}  # (12)
    ...
    def save_df_to_disk(self):  # (4)
        ...
        rename_map = {key: info["full"] for key, info in FIELDS_CONFIG.items()}  # (8)
    ...
    def create_widgets(self):  # (4)
        ...
        for tab_name, fields in TABS_CONFIG.items():  # (8)
            ...
            for idx, key in enumerate(fields):  # (12)
                ...
                lbl = tk.Label(tab_frame, text=info["short"] + ":", bg="#ffffff", anchor="w")  # (16)
                lbl.grid(row=r, column=c, sticky="w", pady=4, padx=(10, 5))  # (16)

                if info["short"] in DICTIONARIES:                                        # (16)
                    # ДОБАВЛЕН ПАРАМЕТР width=25 В КОНЕЦ СКОБКИ:
                    widget = ttk.Combobox(tab_frame, values=DICTIONARIES[info["short"]], state="readonly", width=25) # (20)
                else:                                                                    # (16)
                    # ДОБАВЛЕН ПАРАМЕТР width=27 В КОНЕЦ СКОБКИ:
        ...
        for h in headers:  # (8)
            ...
            self.tree.column(h, width=90, anchor="center")  # (12)
    ...
    def on_row_select(self, event):                                                      # (4)
        ...
        if not selected:                                                                 # (8)
            return                                                                       # (12)
        self.selected_pk = int(selected)                                                 # (8)
        rows = self.df[self.df["id"] == self.selected_pk]                                # (8)
        if rows.empty:                                                                   # (8)
            return                                                                       # (12)
        row_data = rows.iloc[0]                                                          # (8)
        for key, widget in self.form_fields.items():                                     # (8)
            val = row_data[key]                                                          # (12)
            text_val = "" if pd.isna(val) else (str(int(val)) if FIELDS_CONFIG[key]["type"] is int else str(val)) # (12)
